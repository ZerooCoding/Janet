...
const manager = new Discord.ShardingManager('./janet.js', {token: process.env.CLIENT_TOKEN});
...
function LogCount() {
    manager.broadcastEval('this.guilds.cache').then(guilds => {
        ...
        Log(`Janet startup complete, \`${manager.shards.size}\` ${manager.shards.size != 1 ? 'shards' : 'shard'} and \`${guilds.length}\` guilds${guild_list.length < 1935 ? '\n```' + guild_list + '```' : ''}`);
    });
}
...
git.getLastCommit((err, commit) => {
    ...
    Log(`Janet starting, commit \`#${commit.shortHash}\` by \`${commit.committer.name}\`:\n\`${commit.subject}\``);
});