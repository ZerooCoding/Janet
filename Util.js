    ...
    static log(message, files) {
        ...
        if (typeof(message) == 'string') {
            for (let msg of Discord.Util.splitMessage(message, { maxLength: 1980 })) {
                client.send(msg, { avatarURL: Util.config.avatar, username: 'Janet-Logs', files: files });
            }
        }

        else client.send(null, { embeds: [message], avatarURL: Util.config.avatar, username: 'Janet-Logs', files: files });
            ...
            recursive('./cmds', async (err, files) => {
                ...
                for (let file_path of jsfiles) {
                    ...
                    if (Array.isArray(props.help.name)) {
                        for (let item of props.help.name) process.janet.commands.set(item, props);
                    }
                    else process.janet.commands.set(props.help.name, props);
